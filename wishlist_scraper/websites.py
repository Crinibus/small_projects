...
class Websites:
    ...
    def Komplett(link: str):
        ...
        price = soup.find('span', class_='product-price-now').text.strip(',-').replace('.', '')
        return name, price
    ...
    def Proshop(link: str):
        ...
        try:
            # find normal price
            price = soup.find('span', class_='site-currency-attention').text.split(',')[0].replace('.', '')
        except AttributeError:
            try:
                # find discount price
                price = soup.find('div', class_='site-currency-attention site-currency-campaign').text.split(',')[0].replace('.', '')
            except AttributeError:
                # if campaign is sold out (udsolgt)
                price = soup.find('div', class_='site-currency-attention').text.split(',')[0].replace('.', '')
        return name, price
